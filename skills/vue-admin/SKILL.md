---
name: vue-admin
description: Vue3 管理后台开发规范。当开发 Vue3 + Element Plus + TypeScript 管理后台、CRUD 页面、权限菜单、业务表单、自定义组件时使用此 skill。优先遵循项目已有设计系统和组件封装。
---
...
- 先识别项目已有路由、状态管理、请求封装、权限指令、组件库版本和代码风格，再套用本规范。
- 优先复用已有组件、hooks、API 模块和表单/表格封装，不为符合模板重复造组件。
- 前端权限只用于体验优化；所有菜单、按钮、接口权限必须由后端强制校验。
- 新增页面必须处理加载、空状态、错误状态、分页、确认删除和接口异常。
- 修改 UI 时保持响应式、可访问性和现有设计一致，不引入无关视觉重构。
...
按钮权限只负责控制界面显示，不能作为安全边界；对应接口必须在后端校验权限。
