---
name: mysql-design
description: MySQL 数据库设计规范。当设计或评审 MySQL 表结构、字段类型、索引、约束、迁移脚本、命名规范时使用此 skill。生产库变更、删表删字段、大表 DDL 必须先确认备份和回滚方案。
---
...
- 先了解业务查询场景、数据量级、写入频率、唯一性要求和是否多租户，再设计表和索引。
- 生产环境 DDL 必须通过迁移脚本管理，并准备备份、回滚和锁表风险评估。
- 不直接生成 `DROP TABLE`、`TRUNCATE`、批量 `DELETE`、删字段等破坏性 SQL，除非用户明确要求并确认。
- 索引按实际查询条件和排序需求建立，避免为了“规范完整”过度索引。
- 涉及手机号、邮箱、身份证、Token 等敏感数据时，明确脱敏、加密或访问控制策略。
...
以下索引不是每张表都必须创建，应结合查询频率、选择性、排序方式和写入成本取舍。
...
-- 状态索引（仅当状态过滤频繁且选择性足够时）
INDEX `idx_status` (`status`)

-- 创建时间索引（仅当按创建时间范围查询或排序频繁时）
...
- [ ] 外键字段按查询需要添加索引
- [ ] 唯一业务字段添加唯一约束，并考虑软删除字段影响
- [ ] 状态字段仅在高频过滤且选择性合适时添加索引
- [ ] 使用 utf8mb4 字符集
- [ ] 使用 InnoDB 引擎
- [ ] 每个字段添加 COMMENT
- [ ] 表添加 COMMENT
- [ ] 索引数量不超过 5 个
- [ ] 生产 DDL 有迁移脚本、备份方案和回滚方案
- [ ] 大表变更评估锁表、执行时长和在线 DDL 策略
- [ ] 敏感字段明确脱敏、加密或访问控制策略
