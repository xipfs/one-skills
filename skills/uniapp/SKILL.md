---
name: uniapp
description: UniApp 移动端开发规范。当开发 UniApp + Vue3 移动应用、小程序页面、wot-design-uni 组件、移动端 UI/UX、跨端兼容性时使用此 skill。优先遵循项目已有主题变量和组件封装。
---
...
- 先识别项目已有组件库、主题变量、状态管理、请求封装、平台目标和构建方式，再套用本规范。
- 优先使用 wot-design-uni 和项目已有基础组件；官方组件无法满足业务交互时再自定义。
- 所有颜色、间距、圆角、暗黑模式优先走项目主题变量，避免页面级重复定义。
- 小程序端避免直接 DOM API、复杂选择器和不兼容 CSS；涉及平台差异时显式条件编译。
- 新增页面必须处理加载、空状态、错误状态、分页/触底、刷新和接口异常。
...
**能用 wot-design-uni 官方组件或项目已有基础组件，优先复用；仅在无法满足业务交互时自定义**
